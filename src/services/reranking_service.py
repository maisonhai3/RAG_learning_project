    ...
    def rerank(
        ...
        for i, idx in enumerate(selected_indices):
            ...
            new_result = RetrievalResult(
                ...
                metadata=result.metadata,
                relevance_score=1.0
                - (i / len(selected_indices)),  # Decreasing score by rank
            )
            reranked_results.append(new_result)
...
class HybridReranker(BaseReranker):
    ...
    def rerank(
        ...
        sorted_indices = sorted(
            all_scores.keys(), key=lambda x: all_scores[x], reverse=True
        )
...
class RerankingService:
    ...
    def __init__(
        self,
        strategy: RerankingStrategy = RerankingStrategy.DISABLED,
        ...
        match self.strategy:
            ...
            case RerankingStrategy.CROSS_ENCODER:
                try:
                    ...
                    print(
                        "Warning: sentence-transformers not available, disabling cross-encoder re-ranking"
                    )
                    return None
